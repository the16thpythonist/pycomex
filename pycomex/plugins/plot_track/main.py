...
class PlotTrackedElementsPlugin(Plugin):
    ...
    def after_experiment_finalize(self, config: Config, experiment: Experiment):
        ...
        for key in tracked_keys:
            ...
            try:
                ...
                elif isinstance(values[0], str) and os.path.exists(os.path.join(experiment.path, values[0])):
                ...
                experiment.logger.debug(f'! plotting tracked elements "{key}" failed with "{exc}". skipping...')
    ...
    def visualize_numeric_elements(self, 
    ...
    def visualize_image_elements(self, 
                                 experiment: Experiment, 
                                 name: str, 
                                 values: list[str]
                                 ) -> str:
        ...
        """
        images = [imageio.v2.imread(os.path.join(experiment.path, image_path)) for image_path in values]
